...
def find_solutions(m, n, p, case):
    if p == 0:
        return 0

    value = (p ** 2) * m * n
    solutions = 0
    total_div= find_divisors(value)
...
m,n,p=[int(i) for i in stdin.readline().split()]
case=1
while m!=0 and n!=0 and p!=0:
    find_solutions(m,n,p,case)
    case+=1
    m,n,p=[int(i) for i in stdin.readline().split()]
